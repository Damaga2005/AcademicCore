"""F8-P2 canonical documents f8p2-dsp/1 + digests + replay (NEW).

Deterministic, closed, size-guarded envelope over DSP results. The hash
construction is REUSEd — `canonical_json`/`chain_digest` from
`metrology.o5_traceability` (`sha256(tag‖0x00‖canonical)`, sorted keys,
compact separators); this module only pre-converts DSP values
...
class DspDocument:
    """Closed deterministic DSP document (validation-only post-init)."""
    ...
    def create(kind: str, payload: Mapping[str, object],
               provenance: Mapping[str, str] | None = None) -> "DspDocument":
        ...
        prov = tuple(sorted(((str(k), str(v)) for k, v in dict(provenance or {}).items()),
                            key=lambda kv: kv[0]))
        return DspDocument(kind=kind.strip(), payload=items, provenance=prov)
...
def dumps(document: DspDocument) -> str:
    if not isinstance(document, DspDocument):
        raise ControlError(ControlStatus.INVALID, "dumps needs a DspDocument")
...
def loads(text: str) -> DspDocument:
    ...
    try:
        ...
        rebuilt = DspDocument.create(str(doc["kind"]), payload,
...
def compare(first: object, second: object) -> str:
    ...
    try:
        ...
        return INVALID_SERIALIZATION
    if not isinstance(doc_a, DspDocument) or not isinstance(doc_b, DspDocument):
        return INVALID_SERIALIZATION
