"""F8-P4 canonical documents f8p4-comms/1 + digests + replay (NEW).

Deterministic, closed, size-guarded envelope over comms results. The hash
construction is REUSEd — `canonical_json`/`chain_digest` from
`metrology.o5_traceability` (`sha256(tag‖0x00‖canonical)`, sorted keys,
compact separators); this module only pre-converts comms values
(Decimal via `str()` — never `normalize()` — DecimalComplex as
`{re,im}`, Fraction via `str()`) into plain payloads, so no second
canonicalizer or digest engine exists here. Replay states reuse the
F8-N/F8-O/F8-P1/P2/P3 vocabulary verbatim.
"""
...
class CommsDocument:
    """Closed deterministic comms document (validation-only post-init)."""
    ...
    def create(kind: str, payload: Mapping[str, object],
               provenance: Mapping[str, str] | None = None) -> "CommsDocument":
        ...
        prov = tuple(sorted(((str(k), str(v)) for k, v in dict(provenance or {}).items()),
                            key=lambda kv: kv[0]))
        return CommsDocument(kind=kind.strip(), payload=items, provenance=prov)
...
def dumps(document: CommsDocument) -> str:
    if not isinstance(document, CommsDocument):
        raise ControlError(ControlStatus.INVALID, "dumps needs a CommsDocument")
...
def loads(text: str) -> CommsDocument:
    ...
    try:
        ...
        rebuilt = CommsDocument.create(str(doc["kind"]), payload,
...
def compare(first: object, second: object) -> str:
    ...
    try:
        ...
        return INVALID_SERIALIZATION
    if not isinstance(doc_a, CommsDocument) or not isinstance(doc_b, CommsDocument):
        return INVALID_SERIALIZATION
